...
public:
#if MULTIPLE_MOTORS
    Driver (int stbypinA, int a0pin, int a1pin, int pwmApin, int stbypinB, int b0pin, int b1pin, int pwmBpin); // Constructor
    void begin (); // Initialization
    void moveForward (int motor, int direction, float speed); // Methods
    void softStop (int motor);
    void hardStop (int motor);
#else
    Driver (int stbypinA, int a0pin, int a1pin, int pwmApin); // Constructor
    void begin (); // Initialization
...
private:
    uint8_t _stbyPinA; // Standby pin definition
    ...
    uint8_t _PWMAPin;    // PWM pin definition
        ...
        uint8_t _stbyPinB; // Standby pin definition
